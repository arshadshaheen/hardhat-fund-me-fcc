...
module.exports = async ({ getNamedAccounts, deployments }) => {
    const inititalHoderAddress = process.env.MY_ADDRESS;
    ...
    if (!predicateRoleAddress) {
        throw new Error(
            "Environment variable MY_ADDRESS is missing. Ensure it is defined in your .env file."
        );
    }

    console.log(`Deploying DEXTIAN with PREDICATE_ROLE assigned to ${predicateRoleAddress}`);

    try {
        // Deploy the DEXTIAN contract with the predicate address
        const dextian = await deploy(tokenSymbole, {
            from: deployer,
            args: [tokenName, tokenSymbole, predicateRoleAddress, maxSupply, inititalHoderAddress, initialSupply], // Pass the predicate address dynamically
            log: true,
        });

        console.log(`${tokenSymbole} deployed at ${dextian.address}`);
        console.log(
            `Verify the contract using: npx hardhat verify --network <network> ${dextian.address} ${tokenName} ${tokenSymbole} ${predicateRoleAddress}`
        );
    } catch (error) {
